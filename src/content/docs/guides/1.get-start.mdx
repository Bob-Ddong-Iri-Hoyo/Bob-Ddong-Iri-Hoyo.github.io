...
이 런처는 게임 실행에 필요한 Bottle, Wine 버전, 그래픽 변환 레이어를 화면에서 선택하고 관리할 수 있도록 돕습니다.
따라서 복잡한 Wine 명령어와 설정 파일을 직접 다루지 않아도 기본 실행 환경을 구성할 수 있습니다.

Wine을 잘 아는 사용자는 CLI 환경을 직접 사용할 수도 있습니다.
반면 일반 사용자는 런처를 통해 같은 작업을 더 쉽게 수행할 수 있습니다.

## 특징

일반 CrossOver나 Wine 환경에서 Eternal Return을 실행할 때 나타나는 고질적인 렌더링 문제를 줄이는 것을 목표로 합니다.
...
라이선스 전문과 세부 조건은 [LICENSE](https://github.com/Bob-Ddong-Iri-Hoyo/BDIH-Launcher/blob/main/LICENSE)에서 확인할 수 있습니다.
